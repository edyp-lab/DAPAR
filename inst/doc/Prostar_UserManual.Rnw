...
% The user chooses the format of the file and enter the name of the file. 
...
\begin{itemize}
\item If the missing values are mainly due to a censoring and/or random process
of the low intensity proteins, it is advised to use the functions of the 
package \CRANpkg{imp4p}. This package is a collection of proteomic-specific 
...
term (from French "là/pas-là", meaning "here/not-here") refers to analytes 
(peptides or proteins) that are entirely missing in some conditions while they
are (partially or totally) visible in others. There specific accounting in a 
...
it is advised to use KNN ($K$ Nearest Neighbors) from package 
