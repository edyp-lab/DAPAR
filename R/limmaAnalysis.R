...
##' Values are: 'OnevsOne', 'OnevsAll' and 'anova1way'; default is 'OnevsOne'.
...
##' except in the case of one-way ANOVA: there are no multiple comparisons so no
##'  logFC values: there is only one column containing NA.
...
##' @author Thomas Burger, Quentin Giai-Gianetto, Samuel Wieczorek, Hélène Borges
##' @examples
##' utils::data(Exp1_R25_pept, package='DAPARdata')
##' obj <- Exp1_R25_pept
    ...
    if(!is.null(design.matrix)) {
        if(comp.type == 'OnevsOne' || comp.type == "OnevsAll"){
            ...
            fit_pvalue <- limma::topTable(ebayes_fit, sort.by = "none", number = nrow(qData)) %>%
              dplyr::select("anova1way" = P.Value)
            res.l <- list("logFC" = data.frame("anova1way" = matrix(NA, nrow = nrow(fit_pvalue))),
                        "P_Value" = fit_pvalue)
        }
    }
  return(res.l)
