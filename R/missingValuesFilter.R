...
mvFilterGetIndices <- function(obj,
  ...
  if (condition == "None") {
    ...
    keepThat <- which(apply(!DAPAR::is.MV(data), 1, sum) >= 1) # at least one value different of NA
...
mvFilterGetIndices_Marianne <- function(obj,
  ...
  if (condition == "None") {
    ...
    if (isTRUE(percent)) {
      keepThat <- which(rowSums(DAPAR::is.byMSMS(data))/ncol(data) >= threshold) 
    } else {
      keepThat <- which(apply(DAPAR::is.byMSMS(data), 1, sum) >= threshold)
    }
  } else if (condition == "AtLeastOneCond" || condition == "AllCond") {
    ...
    if (isTRUE(percent)) {
      for (c in 1:nbCond) {
        ind <- which(Biobase::pData(obj)$Condition == conditions[c])
        s[,c] <- (rowSums(!DAPAR::is.byMSMS(data[,ind]))/length(ind)) >= threshold
      }
    } else {
      for (c in 1:nbCond) {
        ind <- which(Biobase::pData(obj)$Condition == conditions[c])
        if (length(ind) == 1){
          s[,c] <- (DAPAR::is.byMSMS(data[,ind]) >= threshold) 
        }
        else {
          s[,c] <- (apply(DAPAR::is.byMSMS(data[,ind]), 1, sum)) >= threshold
        }
      }
    }
