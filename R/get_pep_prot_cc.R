...
#' @export
ComputeConnexComposants <- function(X=NULL){
  if( is.null(X)){
    warning("Cannot compute because the matrix X is missing")
    return(NULL)
    }
  
  return (list(all = get.pep.prot.cc(as.matrix(X$all)),
                onlySpec = get.pep.prot.cc(X$onlySpec),
               onlyShared = get.pep.prot.cc(X$onlyShared))
  )
