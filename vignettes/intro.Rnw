...
DAPAR is made to guide the practitioner through the following classical steps of a protein-level data analysis: 
  \begin{itemize}
    ...
    \item \textbf{Missing values imputation}, with the following methods: $(i)$ for random occurences: $k$-nearest-neighbors, Bayesian Principal Component Analysis and Maximum Likelihood Estimation; $(ii)$for left censored missing values: Quantile Regression for Imputation of Left Censored data.
